...
import { StyleSheet, View, Text, TouchableOpacity, Button, Modal } from 'react-native';
...
export default Activity2 = () => {
  ...
  const generateColorArrays = () => {
    ...
    let index3 = Math.floor(Math.random() * colors.length);
    while (index3 === index1 || index3 === index2) {
      index3 = Math.floor(Math.random() * colors.length);
    }
    ...
    return [colors, swappedColors2, swappedColors3].sort(() => Math.random() - 0.5);
  };
  ...
  function handleColorClick(option, index) {
    ...
    if (!correct) {
      setShowModal(true); // Show the modal when an incorrect option is selected
    }
  }

  function handleReplay() {
    setShouldReset(true);
    setShowModal(false); // Close the modal when replay is clicked
  }
      ...
      {randomOptions.map((option, index) => (
        <TouchableOpacity
          ...
          style={{
            ...
            borderColor: selectedOption === index ? (isCorrect ? 'green' : 'red') : 'black',
            borderWidth: selectedOption === index ? responsiveWidth(0.5) : responsiveWidth(0.5),
        ...
        >
          <View style={styles.row}>
            {option.map((color, subIndex) => (
              <View key={subIndex} style={[styles.square, { backgroundColor: color }]} />
            ))}
          </View>
        </TouchableOpacity>
      ))}
      ...
      <Modal visible={showModal} animationType="slide" transparent={true}>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <Text style={[styles.modalText, {marginBottom: responsiveHeight(2)}]}>Please try again.</Text>
            <TouchableOpacity onPress={handleReplay} style={styles.reloadButton}>
              <Text style={styles.reloadButtonText}>OK</Text>
            </TouchableOpacity>
